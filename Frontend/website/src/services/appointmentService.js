...
const API_BASE_URL = import.meta.env.VITE_API_URL || "https://safina-carpet-backend-web-h0o5.onrender.com";
...
export const checkAvailability = async (date) => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/appointments/availability?date=${date}`
    );
    const data = await response.json();
...
export const getAvailableDates = async (days = 30) => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/api/appointments/available-dates?days=${days}`
    );
    const data = await response.json();
...
export const getUserAppointments = async (token, options = {}) => {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/appointments/my?${queryParams}`,
      {
        headers: getAuthHeaders(token),
      }
    );
