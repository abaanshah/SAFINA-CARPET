// ===================================================================
// FILE: src/pages/Login/Login.jsx (Your file, verified and confirmed)
// ===================================================================
...
const Notification = ({ message, type, show }) => {
  ...
  const typeStyle = type === 'success' ? 'bg-green-500' : 'bg-red-800';
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLoginView) {
        // This now correctly calls the login function from your AuthContext
        await login(email, password);
        // The redirect to the homepage OR the admin panel is handled inside the login function
      } else {
        // This correctly calls the signup function
        await signup(name, email, password);
        showNotification('success', 'Verification link sent! Please check your email.');
        handleViewChange(true);
      }
    } catch (err) {
      // If login() or signup() fails, the error is caught and displayed here
          ...
          <div className="w-full md:w-1/2 p-8 md:p-12 bg-white flex flex-col justify-center">
            ...
            <div className="mt-8">
                ...
                <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <button disabled className="w-full inline-flex justify-center items-center py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 cursor-not-allowed opacity-50"><GoogleIcon /> Google</button>
                  <button disabled className="w-full inline-flex justify-center items-center py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 cursor-not-allowed opacity-50"><FacebookIcon /> Facebook</button>
                  <button disabled className="w-full inline-flex justify-center items-center py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 cursor-not-allowed opacity-50">Mobile OTP</button>
                </div>
            </div>
          </div>
        </div>
