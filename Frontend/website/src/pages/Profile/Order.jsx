    ...
    <div className="container mx-auto  mb-[10vh] p-4 min-h-screen">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="mt-2 text-gray-500">Looks like you haven't placed any orders. Let's change that!</p>
          <Link to="/catalog" className="mt-6 inline-block bg-red-800 text-white font-semibold py-3 px-8 rounded-lg hover:bg-red-900 transition-transform transform hover:scale-105">
            Start Shopping
          </Link>
        </div>
      ) : (
            ...
            <div key={order._id} className="bg-white shadow-lg rounded-xl overflow-hidden transition-all duration-300 hover:shadow-2xl">
              ...
              <div className="p-6 cursor-pointer" onClick={() => toggleOrderDetails(order._id)}>
                ...
                <div className="flex justify-between items-center mt-4">
                  ...
                  <p className="text-2xl font-bold text-red-800">₹{order.totalPrice.toLocaleString('en-IN')}</p>
                </div>
              </div>

              {/* Collapsible Order Details */}
              <div className={`transition-all duration-500 ease-in-out max-h-0 overflow-hidden ${expandedOrderId === order._id ? 'max-h-screen' : ''}`}>
                <div className="bg-gray-50 p-6 border-t border-gray-200">
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-200">
                    ...
                    <p className="text-gray-600">{order.shippingAddress.address}, {order.shippingAddress.city}, {order.shippingAddress.postalCode}, {order.shippingAddress.country}</p>
                  </div>
                </div>
              </div>
            </div>
