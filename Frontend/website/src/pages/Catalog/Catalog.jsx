...
const ProductCard = ({ product, handleWishlistToggle, addToCart, isItemInWishlist }) => (
    ...
    >
        ...
        <div className="absolute top-3 right-3 z-10">
            <button onClick={() => handleWishlistToggle(product)} className="bg-white/70 backdrop-blur-sm p-2 rounded-full shadow-md hover:bg-white transition-colors">
        ...
        <div className="p-4 flex flex-col flex-grow">
            ...
            <p className="text-sm text-gray-500 mt-1 capitalize">{product.material}</p>
            <div className="mt-auto pt-4 flex justify-between items-center">
                <p className="text-lg font-bold text-gray-900">₹{product.price.toLocaleString('en-IN')}</p>
                <button onClick={() => addToCart(product)} className="bg-gray-800 text-white p-2 rounded-lg shadow-md hover:bg-black transition-transform hover:scale-105">
                    <ShoppingBag size={20} />
                </button>
            </div>
        </div>
...
// --- Main Catalog Component with Simplified Filters ---
const Catalog = () => {
