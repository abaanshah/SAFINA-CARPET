...
const CardHeader = ({ icon, title, step }) => (
  <div className="flex items-center mb-4">
    <div className="flex-shrink-0 bg-[#5c0b0a] text-white rounded-full h-8 w-8 flex items-center justify-center font-bold text-sm mr-4 shadow-md">
      {step}
    </div>
...
const RugVisualizer = () => {
  ...
  useEffect(() => {
    const fetchAndSetRugImage = async (url) => {
      try {
        ...
        const file = new File([blob], "rug_from_store.jpg", {
          type: blob.type,
        });
        setRugImage(file);
      } catch (error) {
  ...
  const handleSampleRoomSelect = async (roomUrl, roomName) => {
    ...
    try {
      ...
      const blob = await response.blob();
      const file = new File([blob], roomUrl.split("/").pop(), {
        type: blob.type,
      });
      setRoomImage(file);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("http://127.0.0.1:8000/place-rug/", {
        method: "POST",
        body: formData,
      });
      if (!response.ok) {
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          >
            <Card>
              <CardHeader
                icon={<ImageIcon className="text-[#5c0b0a]" />}
                title="Choose Your Room"
                step="1"
              />
              <ImageUploader onImageSelect={handleRoomImageSelect} />
              <SampleRoomGallery
                onRoomSelect={handleSampleRoomSelect}
                selectedRoomName={selectedRoomName}
              />
            </Card>
            <Card>
              <CardHeader
                icon={<Sparkles className="text-[#5c0b0a]" />}
                title="Select Your Rug"
                step="2"
              />
              {rugUrlFromQuery && rugImage ? (
                <div className="text-center p-4 border-2 border-dashed rounded-lg bg-green-50 border-green-300">
                  <p className="font-semibold text-green-800">
                    Rug selected from our store!
                  </p>
                  <img
                    src={URL.createObjectURL(rugImage)}
                    alt="Selected Rug"
                    className="mt-2 w-24 h-24 object-cover mx-auto rounded-md"
                  />
                </div>
              ) : (
                <div className="space-y-4">
                  <ImageUploader
                    onImageSelect={handleRugImageSelect}
                    label="Upload Your Own Rug"
                  />
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-100 transition font-semibold"
                  >
                    <Search size={16} />
          ...
          <motion.div
            className="lg:col-span-8 lg:sticky lg:top-8"
            initial={{ x: 50, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            ...
            <div className="relative w-full aspect-[4/3] bg-white rounded-xl shadow-2xl flex items-center justify-center overflow-hidden border-4 border-white">
              ...
              {isLoading && (
                <div className="absolute inset-0 bg-white/90 backdrop-blur-md z-20 flex flex-col items-center justify-center text-center p-4">
                  ...
                  <p className="mt-6 text-gray-800 font-semibold text-lg">
                    Our AI is rendering your space...
                  </p>
                  <p className="text-sm text-gray-500">
                    This can take a moment. Time remaining:{" "}
              ...
              {!roomImagePreview && !isLoading && (
                <div className="text-center text-gray-500 p-8">
                  ...
                  <p className="mt-2">Start by selecting a room in Step 1.</p>
                </div>
              )}
              {previewImage && !isLoading && (
                <motion.button
                  onClick={handleDownload}
                  className="absolute bottom-4 right-4 z-20 bg-[#5c0b0a] text-white font-bold py-2 px-4 rounded-lg hover:bg-[#4a0908] transition-all shadow-lg flex items-center gap-2"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                >
...
const sampleRooms = [
  ...
  },
  {
    name: "Modern Hall",
    src: sampleRoom4,
    description: "Bright, open hallway",
  },
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {sampleRooms.map((room) => (
        <button
          ...
          className="relative rounded-lg overflow-hidden border-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#5c0b0a] transition-transform hover:scale-105 group"
        >
          <img
            src={room.src}
            alt={room.name}
          ...
          <p className="absolute bottom-1 left-2 text-xs font-bold text-white drop-shadow-md">
            {room.name}
          </p>
