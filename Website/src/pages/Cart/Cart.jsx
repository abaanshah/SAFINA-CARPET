...
const Cart = () => {
  const { cartItems, removeItem, updateQuantity, subtotal, cartCount } = useContext(CartContext);
  ...
  return (
    <div className="container mx-auto mt-[15vh] mb-[10vh] p-4 min-h-screen">
      <h1 className="text-4xl font-bold text-gray-900 mb-8 text-center" style={{ fontFamily: "Jost, sans-serif" }}>
        Your Shopping Cart
      </h1>
            ...
            {cartItems.map(item => (
              ...
              <div key={item._id} className="flex items-center bg-white shadow-md rounded-lg p-4">
                ...
                <div className="flex-grow ml-4">
                  ...
                  <p className="text-sm text-gray-500">Size: {item.size || 'Standard'}, Color: {item.color || 'Default'}</p>
                  <p className="text-md font-bold text-gray-800 mt-1">₹{item.price.toLocaleString('en-IN')}</p>
                </div>
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <div className="flex items-center border border-gray-300 rounded-lg">
                    <button onClick={() => updateQuantity(item._id, item.quantity - 1)} className="p-2 hover:bg-gray-100 transition disabled:opacity-50" disabled={item.quantity <= 1}>
                      <Minus size={16} />
                    </button>
                    <span className="px-4 font-semibold">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item._id, item.quantity + 1)} className="p-2 hover:bg-gray-100 transition">
                      <Plus size={16} />
                    </button>
                  </div>
                  <button onClick={() => removeItem(item._id, item.name)} className="text-gray-500 hover:text-red-600 transition">
                    <Trash2 size={20} />
                  </button>
                </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white shadow-md rounded-lg p-6 sticky top-24">
            ...
            <div className="flex justify-between items-center my-4">
              <span className="text-lg text-gray-600">Subtotal ({cartCount} items)</span>
